...
// Last Update     : updated for DDR3 and 45nm technology
// Last Modified   : Oct 23th 2016
/////////////////////////////////////////////////////////////////
...
`timescale  1ns/10ps
//`include "~ee577/design_pdk/osu_stdcells/lib/tsmc018/lib/osu018_stdcells.v"
...
module tb();
`define IPNUT_FILE_NAME "ddr3_test_pattern3.dat" //Please modify the test pattern name
`define TRACE_FILE_NAME "ddr3_test.trc"  //Not used 
`define DUMP_FILE_NAME  "ddr3_out.dump"
...
`define NULL 0  
  ...
  // reg                  put_cmd;
   ...
   //assign 				#0.1 non_read_cmd_consumed =((CmdFifoHasSpace == 1) && (Cmd != 1) && (Cmd !=3) && (Cmd != 0) && (Cmd != 7));
   assign 				#0.1 read_cmd_consumed = ((CmdFifoHasSpace == 1) && ((Cmd == 1) || (Cmd ==3)));
   assign 				#0.1 nop_consumed = ((Cmd == 0) || (Cmd == 7));
	 
   // define clocks
   initial clk = 0;
   always #0.8 clk = ~clk; // clock freq (e.g., 625MHz -- #0.8 and 380MHz -- #1.3 )
   
//Uncomment for post simulation
initial
$sdf_annotate("./sdf/ddr3_controller.sdf",XCON,,,"TYPICAL","1.0:1.0:1.0", "FROM_MTM");
   ...
   initial
	 begin
		...
		$display("MSG: Waiting for DDR3 to become ready");
		wait (ready);
		// Open Test Pattern File
		//fin = $fopenr(`IPNUT_FILE_NAME);
		fin = $fopen(`IPNUT_FILE_NAME,"r");
   ...
   ddr3_controller XCON (
						 ...
						 .ba_pad				(c0_ba_pad[2:0]),
						 .a_pad				(c0_a_pad[12:0]),
						 .dm_pad				(c0_dm_pad[1:0]),
						 .odt_pad			(c0_odt_pad),
						 .resetbar_pad      (c0_resetbar_pad),
						 // Inouts
						 .dq_pad				(c0_dq_pad[15:0]),
						 .dqs_pad			(c0_dqs_pad[1:0]),
						 .dqsbar_pad			(c0_dqsbar_pad[1:0]),
						 // Inputs
						 .clk					(clk),
						 .reset					(reset),
						 .cmd					(cmd[2:0]),
						 ...
						 .read				(fetching),
						 .initddr				(initddr)
						 //.put_cmd               (put_cmd)//Add signal to control FIFO.
						 );



   mt41j64m16_187 XDDR0 (
						 .ck(c0_ck_pad),
						 .ckbar(c0_ckbar_pad),
						 .cke(c0_cke_pad),
						 .csbar(c0_csbar_pad),
